...
def evaluate_violations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, row in df.iterrows():
        ...
        if categories_str == "Нет нарушений":
            df.at[idx, "Требуемый УЗ"] = "0"  # ← строка
            continue
        ...
        if has_high_risk:
            df.at[idx, "Требуемый УЗ"] = "1"  # ← строка
            continue
        ...
        if has_payment:
            df.at[idx, "Требуемый УЗ"] = "2"  # ← строка
            continue
        ...
        # ====================================================================
        if 1 <= gov_id_count <= 3:
            df.at[idx, "Требуемый УЗ"] = "3"  # ← строка
            continue
        ...
        if 1 <= common_pd_count <= 3:
            df.at[idx, "Требуемый УЗ"] = "4"  # ← строка
            continue
...
def run_scanning(path: str)->pd.DataFrame:
    ...
    try:
        ...
        found_danger_df.to_sql("scan_results", con = conn, if_exists = "replace")
